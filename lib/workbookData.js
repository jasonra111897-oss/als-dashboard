...
const getTeacherField = (teacher, matcher) => {
  const matchedKey = Object.keys(teacher).find((key) => matcher.test(String(key || "")));
  return matchedKey ? teacher[matchedKey] : "";
};
...
const readWorkbook = () => {
  ...
  if (workbookSourceCache && workbookSourceCache.version === version) {
    return workbookSourceCache.workbook;
  }

  const workbook = xlsx.readFile(EXCEL_PATH);
  workbookSourceCache = { version, workbook };
...
const buildDashboardData = () => {
  ...
  teachersData.forEach((teacher) => {
    ...
    division.teacherList.push({
      ...
      serviceFrom: getTeacherField(teacher, /^Date of Service as ALS Teacher/i),
      serviceTo: teacher.__EMPTY || "",
      currentEnrollees: coerceCount(getTeacherField(teacher, /^Updated No\.? of Enroll/i)),
    });
  });
  ...
  const schools = schoolsData
    .map((school) => {
      ...
      return {
        ...
        schoolAddress: String(school.school_address || school.address || "").trim(),
        latitude: school.latitude === "" ? "" : Number(school.latitude),
        longitude: school.longitude === "" ? "" : Number(school.longitude),
        contactNumber: String(school.contact_number || "").trim(),
...
const getDerivedDataCache = () => {
  const version = getWorkbookVersion();

  if (derivedDataCache && derivedDataCache.version === version) {
    return derivedDataCache.value;
  }

  const dashboardData = buildDashboardData();
  const schoolsDirectory = buildSchoolsDirectory();
  const enrollmentData = buildEnrollmentData();
...
module.exports = {
  buildDashboardData: () => getDerivedDataCache().dashboardData,
  buildEnrollmentData: () => getDerivedDataCache().enrollmentData,
  buildSchoolsDirectory: () => getDerivedDataCache().schoolsDirectory,
};
