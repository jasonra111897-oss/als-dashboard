...
const CLCSHA_JSON_PATH = path.join(PROJECT_ROOT, "data", "generated", "clcsha-dataset.json");
...
const clearClcshaCaches = () => {
  workbookCache = null;
  datasetCache = null;
  jsonDatasetCache = null;
};

const readJsonDataset = () => {
  if (!fs.existsSync(CLCSHA_JSON_PATH)) {
    return null;
  }

  const stats = fs.statSync(CLCSHA_JSON_PATH);
  const version = {
    mtimeMs: stats.mtimeMs,
    size: stats.size,
    lastModified: stats.mtime.toISOString(),
  ...
  if (
    jsonDatasetCache &&
    jsonDatasetCache.version.mtimeMs === version.mtimeMs &&
    jsonDatasetCache.version.size === version.size
  ) {
    return jsonDatasetCache.value;
  }

  const value = JSON.parse(fs.readFileSync(CLCSHA_JSON_PATH, "utf8"));
  jsonDatasetCache = {
    version,
    value,
  };
...
const buildClcshaDatasetFromWorkbook = (workbook, version) => {
...
const buildClcshaDatasetFromExcel = () => {
  ...
  const value = buildClcshaDatasetFromWorkbook(workbook, version);
...
const validateClcshaWorkbookBuffer = (buffer) => {
  const workbook = xlsx.read(buffer, { type: "buffer", cellDates: false });
  return buildClcshaDatasetFromWorkbook(workbook, {
    mtimeMs: Date.now(),
    size: buffer.length,
...
const buildDataset = () => {
  if (fs.existsSync(CLCSHA_PATH)) {
    return buildClcshaDatasetFromExcel();
  }

  const jsonDataset = readJsonDataset();
  if (jsonDataset) {
    return jsonDataset;
  }
...
const buildCenterName = (record) =>
  normalizeText(record.clcName) || normalizeText(record.schoolName) || normalizeText(record.clcId);

const buildClcshaCenters = () => {
  const dataset = buildDataset();
  const uniqueCenters = new Map();

  dataset.records.forEach((record) => {
    const division = normalizeText(record.division || record.sheetName);
    const address = normalizeText(record.clcAddress);
    const name = buildCenterName(record);

    if (!division || !address || !name) {
      return;
    }

    const key = normalizeUpper([division, name, address].join("|"));
    ...
    if (!uniqueCenters.has(key)) {
      uniqueCenters.set(key, {
        ...
        division,
        address,
        category: normalizeText(record.clcType || record.clcLocation) || "Community Center",
        contact: normalizeText(record.teacherName) || "Not specified",
        barangay: normalizeText(record.barangay),
        cityMunicipality: normalizeText(record.cityMunicipality),
      });
    }
  });
